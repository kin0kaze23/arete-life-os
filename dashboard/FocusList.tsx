        ...
        <div className="flex items-start gap-4">
          ...
          >
            <div className="flex items-center justify-between gap-2">
              <h4
                className={`text-sm font-black tracking-tight leading-tight ${task.completed ? 'line-through text-slate-500' : 'text-white'}`}
              >
                {task.title}
              </h4>
            ...
            <div className="flex flex-wrap items-center gap-2">
              <span
                className={`text-[9px] px-1.5 py-0.5 rounded font-black border uppercase tracking-widest ${
                  isStrategic
              ...
              {!isExpanded && (
                <span className="text-[9px] text-indigo-400/60 font-black uppercase tracking-widest ml-auto animate-pulse">
                  View Strategy
                </span>
              )}
            </div>
              ...
              <div className="p-4 bg-white/5 rounded-2xl border border-white/5 space-y-4 shadow-inner">
                ...
                {task.steps && task.steps.length > 0 && (
                  <div className="space-y-2">
                    <p className="text-[9px] font-black uppercase tracking-widest text-slate-600/80">
                      Milestone Sequence
                    </p>
                    <div className="grid grid-cols-1 gap-2.5">
                      {task.steps.map((step, idx) => (
                        <div key={idx} className="flex items-center gap-3 group/step">
                          ...
                          <span className="text-[11px] text-slate-400 leading-tight group-hover/step:text-slate-200 transition-colors">
                            {step}
                          </span>
                        </div>
                      ))}
                ...
                {task.definitionOfDone && (
                  <div className="pt-3 border-t border-white/5 flex items-center gap-2">
                    ...
                    <div className="flex-1">
                      <p className="text-[9px] font-black uppercase tracking-widest text-emerald-500/80 mb-0.5 leading-none">
                        Target Outcome
                      </p>
          ...
          {strategicTasks.length > 0 ? (
            ...
            <div className="text-center py-6 border border-dashed border-indigo-500/20 rounded-2xl bg-indigo-500/[0.02]">
              <p className="text-[10px] font-black text-indigo-400/60 uppercase tracking-widest leading-none">
                High Priority Cleared
              </p>
              <p className="text-[9px] text-slate-600 mt-2">
                Executive focus shifting to secondary objectives.
              </p>
            </div>
          ) : null}
        </div>
