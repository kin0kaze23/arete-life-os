...
- [2026-01-30] **CSP tightening requires build-time CSS** (remove CDN + inline styles before dropping `unsafe-inline`).
- [2026-01-30] **Schema + payload caps** prevent oversized AI requests without mutating user intent.
- [2026-01-31] **Backup MVP needs rate limits + retention** even for encrypted blobs to avoid abuse and cost spikes.
