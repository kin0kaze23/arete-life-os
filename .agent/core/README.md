...
- Main sync helper: `./scripts/sync-main.sh`
...
1. ✅ **Data source verified**: The useAura property this component reads ALREADY EXISTS (check return type)
2. ✅ Type defined in `data/types.ts` (if new type needed)
3. ✅ State hook in `useAura.ts` (if new state needed)
4. ✅ Added to `VaultData` interface + `buildDefaultVault` + `applyVaultData` + `getVaultSnapshot` (if persisted)
5. ✅ **Rendered in parent component** with real props wired (not mock data)
6. ✅ **Added to barrel export** (`index.ts`) if imported elsewhere
7. ✅ **`npm run typecheck` passes** (NOT just build — Vite skips type checking)
8. ✅ **Visual verification**: Component renders in browser with real data or proper empty state
9. ✅ **Import chain verified**: `App.tsx` → parent → component is traceable
...
8. **Vite build ≠ type safety**: `npm run build` succeeds even with 35+ TypeScript errors. Always run `npm run typecheck` — this is the ONLY reliable type gate
9. **Phantom useAura properties**: NEVER reference properties on `useAura()` that don't exist in its return type. Always verify by reading the hook's return statement before coding against it
10. **Mock data trap**: Hardcoded demo data in components masks integration failures. Use real data from useAura or show empty state — never ship mock data as "done"
11. **Orphaned components**: Creating a `.tsx` file is NOT the same as shipping a feature. The component must be imported, rendered, and visually verified in the running app
