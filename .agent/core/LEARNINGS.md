...
## 2026-01-30: Security Hardening + CSP/Tailwind Migration

### Summary

Implemented immediate security hardening: CSP tightening with report-only endpoint, payload schema validation + size limits, expanded secret/PII redaction, passphrase guidance with progressive lockouts, and security audit logging. Migrated Tailwind from CDN to build-time to remove inline styles.

### Key Learnings

- **CSP tightening depends on build-time CSS**: inline styles and CDN scripts block removing `unsafe-inline`/`unsafe-eval`.
- **Schema + payload caps** protect AI endpoints without changing user text.
- **Humane lockouts** reduce brute-force risk without trapping legitimate users.

### Prevention

- Keep CSP report-only active and review violations before further tightening.
- Enforce request-size limits + schema validation on new API endpoints.

---

## 2026-01-31: Agent Intelligence Upgrade (Critical Skills & Behavioral Embedding)

### Issue

The agent lacked granular intelligence in key areas (Performance, Data Architecture, Testing, API Design, Error Resilience) and tended to forget to apply UI/UX standards unless prompted.

### Solution

1. **Standardized Knowledge**: Created 11 structured `.agent/skills/` folders.
2. **Critical Gaps Filled**: Added 5 new skills based on industry research (Next.js 14, Playwright, web.dev).
3. **Behavioral Shift (Skill Gate)**: Hard-coded a mandate in `AGENT.md` that forces the agent to read relevant skills before any Design, Strategic, or Verification phase.
4. **Semantic Design**: Upgraded `visual-intelligence` from raw hex codes to "Atmosphere & Energy" and "Visual Atoms" to ensure aesthetic consistency in LLM-generated UI.

### Key Learnings

- **Static knowledge is ignored; behavioral mandates are followed**: Putting rules in `AGENT.md` works better than just having `SKILL.md` files alone.
- **Evocative language improves UI generation**: LLMs respond better to "Minimalist Calm" and "Aura Primary" than to specific CSS properties alone.
- **Portability requires standardization**: The folder-based skill structure makes it trivial to port this entire intelligence suite to new projects.

### Prevention

- Keep `AGENT.md` Skill Gate updated if new critical phases are identified.
- Audit skills monthly to ensure they stay relevant to current tech stack evolution.

---

## 2026-01-31: Encrypted Backup MVP (Vercel Blob)

### Summary

Shipped encrypted backup flow with Vercel Blob (10 MB cap, 10 versions, 90-day retention), recovery code + passphrase identity, restore rate limits, and a minimal Security panel for audit logs + CSP summary.

### Key Learnings

- **Rate limits + retention are mandatory** for encrypted blob backups to prevent abuse and runaway costs.
- **Recovery code + passphrase** keeps the model self-custody without adding email/SSO.

### Prevention

- Keep restore limits enforced server-side even if client backoff exists.
- Avoid adding plaintext metadata to backup payloads.
