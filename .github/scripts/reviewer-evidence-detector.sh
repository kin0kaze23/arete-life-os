...
POLICY_FILE="$SCRIPT_DIR/../../.opencode/config/reviewer-trust-policy.yaml"
...
if [[ -f "$POLICY_FILE" ]]; then
  ...
  # Parse trusted_labels list
  TRUSTED_LABELS=$(sed -n '/^trusted_labels:/,/^$/p' "$POLICY_FILE" 2>/dev/null | grep -E '^\s+-' | sed 's/.*- *//' | tr '\n' ',' | sed 's/,$//' || echo "reviewer-approved")
  # Parse trusted_reviewers list
  TRUSTED_REVIEWERS=$(sed -n '/^trusted_reviewers:/,/^$/p' "$POLICY_FILE" 2>/dev/null | grep -E '^\s+-' | sed 's/.*- *//' | tr '\n' ',' | sed 's/,$//' || echo "")
fi
...
if [[ "$EVIDENCE_FOUND" == "false" && "$ALLOW_LABEL_EVIDENCE" == "true" ]]; then
  ...
  if [[ -n "$LABELS_JSON" && "$LABELS_JSON" != "null" ]]; then
    ...
    for label in "${LABEL_ARRAY[@]}"; do
      ...
      if command -v jq &>/dev/null; then
        HAS_LABEL=$(echo "$LABELS_JSON" | jq -r '.labels[].name' 2>/dev/null | grep -qi "$label" && echo "true" || echo "false")
      else
        HAS_LABEL=$(echo "$LABELS_JSON" | grep -qi "$label" && echo "true" || echo "false")
      fi
